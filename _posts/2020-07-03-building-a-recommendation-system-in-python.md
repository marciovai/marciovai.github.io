...
# Introduction
One of the most common applications of Machine Learning that impact our daily lives are from Recommendation Systems. In this post we will walk through the steps towards building a simple Recommendation System in Python that works based on past user-item interactions and can be used in a range of different applications.  
Here we will build a recommendation system in Python using Cosine Similarity for a _Collaborative-Filtering_ approach, in this case we can't specifically call it Machine Learning because there will be no Gradient Descent or any other type of hyperparameter involved. What we will do is just preprocess our data in a smart way and apply some calculation steps.

The full source code is available [here](https://github.com/marciovai/recommendation_system).

# How it works
There are two main paths that we can choose in order to solve recommendation-type problem. They are the *Content-based* and *Collaborative-Filtering*. Both have it's strenghts and drawbacks as well as varying levels of complexity. Let's take a quick look on each of them.
...
For example suppose we want to apply our algorithm to an e-commerce store, data available for our problem would be:
...
Below is the formula we will be using for calculating the Cosine Similarity:
...
We could write our own implementation to calculate the Cosine Similarity for each row in the dataset, but luckly for us *sklearn* already has it implemented so we will use it.
...
There are a few things happening in the code below that are worth noting:
- We get only a sample of movies from each similar user (5 in this case) because this dataset is somewhat dense in the sense that on average users rated at least 20 movies each. By taking samples we get a balanced set of movies from all top k the similar users.
